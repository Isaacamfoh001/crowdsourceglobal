...
export function AskVendorResolutionButton({ caseId, label }: { caseId: string; label: string }) {
  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setOpen(true)} className="inline-flex items-center gap-1.5 text-sm font-medium text-brand-700 hover:underline">
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 rounded-xl border border-stone-200 bg-stone-50 p-4">
      ...
      <p className="text-xs text-stone-400">This goes to CrownSourceGlobal, not directly to the customer.</p>
    </form>
  );
}
