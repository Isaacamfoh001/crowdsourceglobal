...
import {
  approveManufacturerApplicationAction,
  requestManufacturerApplicationChangesAction,
  rejectManufacturerApplicationAction,
} from "../../lib/actions/admin";

/** Mirrors BeautyProfessionalDecisionForms.tsx exactly — same three-decision shape, targeting a manufacturer upgrade request (M32.8). */
export function ManufacturerApplicationDecisionForms({ applicationId }: { applicationId: string }) {
  const [approveState, approveAction, approvePending] = useActionState(approveManufacturerApplicationAction, null);
  const [changesState, changesAction, changesPending] = useActionState(requestManufacturerApplicationChangesAction, null);
  const [rejectState, rejectAction, rejectPending] = useActionState(rejectManufacturerApplicationAction, null);
    ...
    <div className="flex flex-col gap-4">
      ...
      <form action={approveAction}>
        ...
        <Button type="submit" size="lg" disabled={approvePending} className="w-full sm:w-auto">
          {approvePending ? "Approving…" : "Approve — grant Manufacturer access"}
        </Button>
      </form>
