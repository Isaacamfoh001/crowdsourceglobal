    ...
    <div className="flex flex-col gap-6">
      ...
      {fulfilment.openIssue ? (
        <div className="rounded-2xl border border-danger-200 bg-danger-50 p-5">
          <p className="text-sm font-medium text-danger-800">Open issue: {fulfilment.openIssue.category}</p>
          <p className="mt-1 text-sm text-danger-700">{fulfilment.openIssue.description}</p>
          <div className="mt-3">
      ...
      {!international ? (
        <div className="rounded-2xl border border-ivory-300 bg-white p-5">
          ...
          {fulfilment.vendorPickup.notes ? <p className="mt-2 text-sm text-espresso-900/65">{fulfilment.vendorPickup.notes}</p> : null}
        </div>
      ) : null}
      ...
      <div className="rounded-2xl border border-ivory-300 bg-white p-5">
        ...
        {international && awaitingHandoff && !vendorHasShipped ? (
          <div className="mt-3 flex flex-col gap-3">
            ...
            />
            <p className="text-xs text-espresso-900/35">Waiting for the vendor to ship to this destination.</p>
          </div>
        ) : null}
        ...
        {!international && awaitingHandoff && fulfilment.status !== "READY" ? (
          <p className="mt-3 text-sm text-espresso-900/50">Waiting for the vendor to mark this ready for collection.</p>
        ) : null}

        {!awaitingHandoff && shipmentStatus !== "DELIVERED" && shipmentStatus !== "DELIVERY_FAILED" ? (
          <div className="mt-3 flex flex-col gap-3">
            <p className="text-sm text-espresso-900/65">Current shipment status: {shipmentStatus.replace(/_/g, " ").toLowerCase()}</p>
