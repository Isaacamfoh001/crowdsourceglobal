...
import { toVendorEarningDetailDTO } from "../../../../../../../lib/api/dto/vendor";
import { apiError, apiSuccess } from "../../../../../../../lib/api/response";

type Params = { id: string };

/** GET /api/v1/vendor/finance/earnings/:id (M27) — scoped to this vendor only (multi-vendor earnings privacy). */
export async function GET(_request: Request, { params }: { params: Promise<Params> }) {
  ...
  const result = await vendorFinanceService.getEarningDetailForVendor(context.vendorId, id);
  if (!result.ok) return apiError("NOT_FOUND", result.error);
  return apiSuccess(toVendorEarningDetailDTO(result.value));
}
