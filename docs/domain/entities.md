...
**Refund** *(the real M9 model — supersedes the pre-M9 placeholder above)* — id `PK`, resolutionCaseId `FK→ResolutionCase IDX`, orderId `FK→Order IDX`, paymentId `FK→Payment` (nullable), itemsAmount (decimal, sum of covered ResolutionCaseItems' approvedRefundAmount), deliveryFeeAmount (decimal, default 0 — a future-ready field; no delivery fee is currently represented in `Order.total` at all, so this stays inert until that changes, per the explicit "don't invent policy that doesn't exist yet" instruction), amount (decimal, `itemsAmount + deliveryFeeAmount` — the actual value executed), currency, status (`RefundStatus`: `PENDING_APPROVAL|APPROVED|PROCESSING|COMPLETED|FAILED`), providerEventId `UQ` (nullable), approvedAt/processedAt (nullable), failureReason (nullable), createdAt, updatedAt. `@@index([status, createdAt])`, `@@index([resolutionCaseId])`. **Decision, not execution** — see `/docs/architecture/overview.md`.
