...
| API strategy | Server actions/route handlers for the web app; versioned `/api/v1` (M18.1) as the stable, authenticated HTTP contract for external consumers — currently the native mobile client. Payment-provider webhooks stay unversioned at `/api/payments/*/webhook` — they're a provider integration surface, not part of this client-facing contract | See "Mobile API Foundation" below |
...
| Background jobs | `EmailDeliveryJob` table + polling drain (`lib/email-worker.ts`, `scripts/process-email-jobs.ts`, M7); abandoned-payment sweep (`scripts/sweep-abandoned-payments.ts`, M10A); vendor-earnings eligibility sweep (`scripts/sweep-earnings-eligibility.ts`, M11). Scheduled in production via Railway scheduled services (M13.2) — every 5/5/60 minutes respectively, see `docs/deployment/railway.md` | Narrow, purpose-built jobs rather than a generic `BackgroundJob` table — each is a direct query against its own source-of-truth rows, not a generic queue |
...
| Hosting | Railway — Web Service (app) + Railway PostgreSQL + three scheduled job services (M13.2, changed from Render — ADR 0012) | One platform for app, database, and scheduled jobs — see `docs/deployment/railway.md` and ADR 0012 |
| Rate limiting | Better Auth's built-in limiter (`storage: "database"`, `lib/auth.ts`) for its own routes; `lib/rate-limit.ts` (Postgres-backed, `ActionRateLimit` table) for payment/OTP/checkout server actions (M13, client-IP header preference adapted for Railway in M13.2) | No Redis — see ADR 0011 for the justification against CLAUDE.md §17, ADR 0012 for the Railway-specific IP-trust update |
...
## Mobile API Foundation *(implemented M18.1)*

**Purpose.** `/api/v1` is the versioned, authenticated HTTP contract every native mobile endpoint is built against — not a second business layer. A route handler under `app/api/v1/**` is a thin wrapper that resolves identity, calls an existing `modules/*` service unchanged, and shapes the response; it must never contain a business rule a web page/Server Action doesn't already enforce via the same service.

**Authorization convention — the one rule every `/api/v1` route must follow.** The existing per-module session/membership guards (`requireSession` in `modules/identity/policy.ts`, `requireVendorPortalContext` in `modules/vendors/policy.ts`, `requireAdminSession` in `modules/administration/policy.ts`) throw a Next.js `redirect()`/`notFound()` on failure — correct for a page or Server Action, but wrong for a JSON API (a native client needs a real `401`/`403`/`404` body, not an HTTP redirect). `/api/v1` routes must therefore call only the **non-redirecting, data-returning** identity primitives that already exist alongside those guards for exactly this reason — `getCurrentSession`/`getCurrentCustomerProfile` (`modules/identity/policy.ts`), `getVendorPortalContext`/`vendorsService.listMembershipsForUser` (`modules/vendors`), `getAdminContext` (`modules/administration/policy.ts`) — the same primitives the pre-existing `app/api/quotations/[id]/pdf`, `app/api/sourcing/attachments/[id]`, and `app/api/resolutions/attachments/[id]` routes were already built on before M18 existed. The redirect-based guards are never modified to accommodate this — they keep serving pages/Server Actions exactly as before.

**Response shape.** `lib/api/response.ts` — a handful of pure functions, not a framework: `apiSuccess(data)` → `{ "data": ... }`; `apiError(code, message)` → `{ "error": { "code", "message" } }` with the HTTP status derived from a fixed `ApiErrorCode` → status map (`UNAUTHORIZED` 401, `FORBIDDEN` 403, `NOT_FOUND` 404, `VALIDATION_ERROR` 422, `INTERNAL_ERROR` 500); `serializeMoney(decimal, currency)` → `{ amount: "<fixed 2dp string>", currency }`, never a JS float; `serializeDate(date)` → ISO-8601 string; `apiPage({rows, total, page, pageSize})` wraps `lib/pagination.ts`'s existing `skip`/`take` + `count()` convention for a JSON response — pagination itself is not reinvented, only its HTTP envelope. No `/api/v1` route serializes a raw Prisma row — every response is a deliberate, hand-written DTO.

**Native/Expo authentication.** The installed Better Auth version is 1.6.30 (bumped from 1.6.29 in M20.1 — a same-line patch release with no schema or API changes, see below). No bundled React Native/Expo client is installed yet (that's a separate `@better-auth/expo` package — there is no Expo repository yet to install it into). The server-side mechanism that makes native authentication possible regardless is the `bearer()` plugin (`better-auth/plugins`, registered in `lib/auth.ts` ahead of the required-last `nextCookies()`): on sign-in/sign-up it echoes the session token as a `set-auth-token` response header in addition to the normal `Set-Cookie`; on every later request, a valid `Authorization: Bearer <token>` header is transparently rewritten into the same session-cookie header `auth.api.getSession()` already reads — so `getCurrentSession()`, and therefore every `/api/v1` route built on it, needs no special-casing for cookie- vs. bearer-authenticated callers. The plugin only activates when an `Authorization` header is present, so existing cookie-based browser/PWA requests are completely unaffected. A future Expo client's expected flow: `sign-in (email/password today; Google, then Phone OTP once M17.2 lands) → capture the response's session token → persist it in secure storage → send it back as Authorization: Bearer <token> on every request`.

**Better Auth version M20.1 finding.** The latest `@better-auth/expo` (1.7.2) requires `better-auth ^1.7.2`, and Better Auth 1.7.0 is a major release that renames/rescopes `Account` identity around a new required `issuer` column — every existing `Account` row (email/password and Google alike) needs a manual, collision-checked backfill before that column can go `NOT NULL`, per Better Auth's own 1.7 upgrade guide. That is a real production migration, not a drop-in upgrade, and was explicitly out of scope for M20.1 (see `docs/decisions/` — no ADR was needed because M20.1 did not take this path). Instead, M20.1 verified that `@better-auth/expo` publishes version-matched releases on the *old* line too: `@better-auth/expo@1.6.30` requires only `better-auth ^1.6.30` — no schema change. Better Auth 1.6.30 is the last patch before 1.7.0 (one bugfix: an async cold-start race condition) and was installed as the M20.1 upgrade. This unblocks M20.2 (installing `@better-auth/expo@1.6.30` + its server-side `expo()` plugin in the mobile repo and here, respectively) without touching production Account/Session rows. The 1.7.x line — and its Account-identity migration — remains a deliberate future decision, not something M20.2 is forced into by the mere existence of a mobile client.

**`trustedOrigins`/CORS.** No code change was needed. Better Auth's own origin/CSRF check (`validateOrigin`, in `better-auth`'s origin-check middleware) only runs when the incoming request carries a `Cookie` header — a bearer-token-only native request never does, so it bypasses origin validation entirely by construction; this is correct, since CSRF is a cookie/browser attack class that doesn't apply to a client manually attaching its own `Authorization` header. The one place `trustedOrigins` genuinely matters for mobile is validating a `callbackURL` on a social (Google) sign-in request, which is out of scope until M17.2/mobile Google sign-in is built and the Expo app's URL scheme exists. Better Auth already supports a zero-code, env-driven mechanism for that day — a comma-separated `BETTER_AUTH_TRUSTED_ORIGINS` environment variable is read automatically (`getTrustedOrigins`, `better-auth/dist/context/helpers.mjs`) in addition to whatever `trustedOrigins` is configured in `lib/auth.ts` (currently unset, defaulting to `BETTER_AUTH_URL`'s own origin) — so adding the Expo scheme later is an operational/environment change, not a code change. No CORS headers or middleware were added — native `fetch` is not subject to browser CORS, and no evidence of a WebView-based call path exists to require it.

**Reference implementation.** `GET /api/v1/me` (`app/api/v1/me/route.ts`) is the first endpoint and the template for every later one: authenticate via `getCurrentSession()` → 401 if absent; resolve customer/vendor/vendor-application state via existing services (`getCurrentCustomerProfile`, `vendorsService.listMembershipsForUser`, `vendorApplicationsService.getForUser`) → compose a hand-written DTO reflecting the existing capability model (`User → CustomerProfile` always, plus zero or more `VendorMembership`) rather than any new CUSTOMER/VENDOR role field.
